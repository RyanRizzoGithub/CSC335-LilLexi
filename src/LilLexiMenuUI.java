...
public class LilLexiMenuUI {
	...
	public void start() {
		...
		MenuItem fileMenuHeader, insertMenuHeader, helpMenuHeader, fileExitItem, fileSaveItem, fileLoadItem, helpGetHelpItem;
        ...
        // Create a new shell
        Shell saveFileShell = new Shell(display, SWT.NO_TRIM);
        saveFileShell.setSize(100, 130);
        saveFileShell.setLayout(new GridLayout());
        
        // Add label to ask for file name
        Label fileName = new Label(saveFileShell, SWT.NONE);
        fileName.setText("Name: ");
        Text setName = new Text(saveFileShell, SWT.NONE);
        setName.setSize(50, 10);
        
        // Save button and cancel button
        Button saveFile = new Button(saveFileShell, SWT.PUSH);
        saveFile.setText("Save");
        Button cancelFile = new Button(saveFileShell, SWT.PUSH);
        cancelFile.setText("Cancel");
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - //
        
        
        // - - - - - - Interface for loading a file - - - - - - - - - - - - //
        Shell loadFileShell = new Shell(display, SWT.NO_TRIM);
        loadFileShell.setSize(150, 150);
        loadFileShell.setLayout(new GridLayout());
        
        Label fileSelect = new Label(loadFileShell, SWT.NONE);
        fileSelect.setText("File: ");
        
        // Find all files in docs folder
 		File files = new File("docs");
 	    File filesFolder = new File(files.getAbsolutePath());
 	       
 	    File[] listOfDocs = filesFolder.listFiles();
 	    ...
 	    Combo fileCombo = new Combo(loadFileShell, SWT.PUSH);
 	    fileCombo.setItems(docName);
 	    
 	    Button loadFile = new Button(loadFileShell, SWT.PUSH);
 	    loadFile.setText("Load");
 	    
 	    Button cancelLoad = new Button(loadFileShell, SWT.PUSH);
	    ...
	    fileSaveItem.addSelectionListener(new SelectionListener() {
	    	public void widgetSelected(SelectionEvent event) {
	    		saveFileShell.open();
	    		saveFileShell.setVisible(true);
	    	}
	    	public void widgetDefaultSelected(SelectionEvent event) {
	    	}	    		
	    });
	    
	    // Selection listener for loading a file
	    fileLoadItem.addSelectionListener(new SelectionListener() {
	    	public void widgetSelected(SelectionEvent event) {
	    		loadFileShell.open();
	    		loadFileShell.setVisible(true);
	    	}
	    	public void widgetDefaultSelected(SelectionEvent event) {}
	    });
	    
	    // Selection listener for loading the file
	    saveFile.addSelectionListener(new SelectionListener() {
	    	public void widgetSelected(SelectionEvent event) {
	    		LilLexiLoader lexiLoader = new LilLexiLoader(display);
	    		try {
	    			if (!setName.getText().equals("")) {
	    				lexiLoader.saveDoc(currentDoc, setName.getText());
	    				saveFileShell.setVisible(false);
	    			}
	    		} catch (IOException e) {
	    			e.printStackTrace();
	    		}
	    ...
	    // Selection listener for closing the save file interface
	    cancelFile.addSelectionListener(new SelectionListener() {
	    	public void widgetSelected(SelectionEvent event) {
	    		saveFileShell.setVisible(false);
	    	}
	    	public void widgetDefaultSelected(SelectionEvent event) {}
	    ...
	    cancelLoad.addSelectionListener(new SelectionListener() {
	    	public void widgetSelected(SelectionEvent event) {
	    		loadFileShell.setVisible(false);
	    	}
	    	public void widgetDefaultSelected(SelectionEvent event) {}
	    });
