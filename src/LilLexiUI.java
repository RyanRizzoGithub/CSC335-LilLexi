...
public class LilLexiUI {
	...
	{	
		...
		canvas.addPaintListener(e -> {
			...
			int backgroundColor = getColorCode(currentDoc.getCurrBackgroundColor());
			Color bgColor = display.getSystemColor(backgroundColor);
			if (backgroundColor == 12) {
				bgColor = new Color(display, 255, 127, 0);
			}
    		...
    		for (int i = 0; i < glyphs.size(); i++) {
    			...
    			if (glyphs.get(i).getChar() == 13 || glyphs.get(i).getChar() == '`') {
    				column = 0;
    				newline = true;
    			}
        ...
        LilLexiMenuUI menu = new LilLexiMenuUI(display, shell, upperComp, lexiControl, canvas, spellCheck, currentDoc);
        menu.start();
	 ...
	 */
	public static int getMarginCode(String margin) {
		// 0 inch
		if (margin.equals("0''")) {
			return 0;
		}
	...
	/* - - - - - - GET DISPLAY - - - - - - - - - - - - - - - - - - - - - - -
	 * Ryan Rizzo
	 * Returns the display object used by the interface
	 * 
